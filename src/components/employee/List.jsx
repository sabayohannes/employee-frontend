...
const List = () => {
  ...
  useEffect(() => {
    const fetchEmployee = async () => {
      setEmpLoading(true);
      try {
        const response = await axios.get("https://employee-api-theta.vercel.app/api/employee", {
          headers: {
        ...
        if (response.data.success) {
          const data = response.data.employees.map((emp, index) => ({
            ...
            profileImage: emp.userId?.profileImage ? (
              <img width={60} className='rounded-full' src={`https://employee-api-theta.vercel.app/${emp.userId.profileImage}`} />
            ) : (
